...
const EditPage = () => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Convert images to base64 if files are selected
      let profileImageData = profileImage;
      let bannerImageData = bannerImage;

      if (profileImageFile) {
        profileImageData = await convertFileToBase64(profileImageFile);
      }

      if (bannerImageFile) {
        bannerImageData = await convertFileToBase64(bannerImageFile);
      }
